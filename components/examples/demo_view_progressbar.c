...
rtgui_container_t *demo_view_progressbar(void)
{
	...
	rtgui_container_add_child(container, hbar);
	...
	rtgui_container_add_child(container, vbar);

	bar_timer = rtgui_timer_create(50, RT_TIMER_FLAG_PERIODIC,
										progressbar_timeout, RT_NULL);

	rtgui_widget_set_onshow(container, start_timer);
	rtgui_widget_set_onhide(container, stop_timer);
