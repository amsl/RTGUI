...
static void rtgui_freetype_font_draw_text(struct rtgui_font* font, rtgui_dc_t* dc, const char* text, rt_ubase_t len, rtgui_rect_t* rect);
...
static void rtgui_freetype_font_draw_text(struct rtgui_font* font, rtgui_dc_t* dc, const char* text, rt_ubase_t len, rtgui_rect_t* rect)
{
	...
	text_short = (rt_uint16_t*)rt_malloc((len + 1)* 2);
...
static void rtgui_freetype_font_get_metrics(struct rtgui_font* font, const char* text, rtgui_rect_t* rect)
{
	...
	len = strlen(text);
	memset(rect, 0, sizeof(rtgui_rect_t));
...
struct rtgui_font* rtgui_freetype_font_create(const char* filename, int bold, int italic, rt_size_t size)
{
	FT_Error err = 0;
	struct rtgui_font* font;

	font = (struct rtgui_font*) rt_malloc(sizeof(struct rtgui_font));
	if(font != RT_NULL)
	{
		struct rtgui_freetype_font* freetype;

		freetype = (struct rtgui_freetype_font*) rt_malloc(sizeof(struct rtgui_freetype_font));
		if(freetype == RT_NULL)
		{
			rt_free(font);
			font = RT_NULL;
...
void rtgui_freetype_font_destroy(struct rtgui_font* font)
{
	struct rtgui_freetype_font* freetype;
