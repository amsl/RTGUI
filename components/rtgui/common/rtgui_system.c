...
void rtgui_system_server_init(void)
{
	...
	rtgui_theme_init();
	/* the graphic device must be set before initialization */
	RT_ASSERT(rtgui_gdev_get() != RT_NULL);
...
static void rtgui_event_dump(rt_thread_t tid, rtgui_event_t* event)
{
	...
	case RTGUI_EVENT_MOUSE_MOTION:
	{
		...
		else if(mouse->button & RTGUI_MOUSE_BUTTON_RIGHT) rt_kprintf("right ");

		if(mouse->button & RTGUI_MOUSE_BUTTON_DOWN) rt_kprintf("down ");
		else if(mouse->button & RTGUI_MOUSE_BUTTON_UP) rt_kprintf("up ");
...
rt_err_t rtgui_recv(void)
{
	...
	r = rt_mq_recv(app->mq, app->event_buffer, RTGUI_EVENT_SIZE, RT_WAITING_FOREVER);
...
rt_err_t rtgui_recv_nosuspend(void)
{
	...
	r = rt_mq_recv(app->mq, app->event_buffer, RTGUI_EVENT_SIZE, 0);
...
rt_err_t rtgui_recv_filter(rt_uint32_t type)
{
	...
	while(rt_mq_recv(app->mq, app->event_buffer, RTGUI_EVENT_SIZE, RT_WAITING_FOREVER) == RT_EOK)
	{
		if(((rtgui_event_t*)(app->event_buffer))->type == type)
		{
			return RT_EOK;
		}
		else
		{
			...
			if(app->widget != RT_NULL && RTGUI_WIDGET_EVENT_HANDLE(app->widget) != RT_NULL)
			{
				RTGUI_WIDGET_EVENT_CALL(app->widget, (rtgui_event_t*)app->event_buffer);
			}
		}
	}
...
static void rtgui_time_out(void* parameter)
{
	...
	*/
	...
	rtgui_send(timer->tid, &(event.parent), sizeof(struct rtgui_event_timer));
}
...
rtgui_timer_t* rtgui_timer_create(const char *name, rt_int32_t time, rt_int32_t flag, rtgui_timeout_func timeout, void* parameter)
{
	...
	timer = (rtgui_timer_t*) rt_malloc(sizeof(rtgui_timer_t));
	timer->tid = rt_thread_self();
	timer->timeout = timeout;
	timer->user_data = parameter;
	timer->active = 0;
	/* init rt-thread timer */
	rt_timer_init(&(timer->timer), name, rtgui_time_out, timer, time, (rt_uint8_t)flag);
