...
void rtgui_theme_draw_win(struct rtgui_topwin* win)
{
	...
	/* begin drawing */
	dc = rtgui_dc_begin_drawing(win->title);
	if (dc == RT_NULL) return;

	/* get rect */
	rtgui_widget_get_rect(win->title, &rect);
...
void rtgui_theme_draw_radiobutton(struct rtgui_radiobox* radiobox, rt_uint16_t item)
{
	...
	/* begin drawing */
	dc = rtgui_dc_begin_drawing(radiobox);
	if (dc == RT_NULL) return;
	/* get widget rect */
...
void rtgui_theme_draw_staticline(struct rtgui_staticline* staticline)
{
	...
	/* begin drawing */
	dc = rtgui_dc_begin_drawing(staticline);
	if (dc == RT_NULL) return ;

	rtgui_widget_get_rect(staticline, &rect);
	rtgui_dc_fill_rect(dc, &rect);
