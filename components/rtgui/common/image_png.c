...
static rt_bool_t rtgui_image_png_process(png_structp png_ptr, png_infop info_ptr, struct rtgui_image_png* png)
{
	...
	case PNG_COLOR_TYPE_RGBA:
		for(y = 0; y < info_ptr->height; y++)
		{
			...
			{
				data = &(row[x * 4]);
				ptr[x+y*info_ptr->width] = RTGUI_ARGB(data[0], data[1], data[2], data[3]);
			}
		}
	...
	case PNG_COLOR_TYPE_PALETTE:
		for(y = 0; y < info_ptr->height; y++)
		{
			...
			for(x = 0; x < info_ptr->width; x++)
			{
				...
				ptr[x] = RTGUI_ARGB(info_ptr->palette[data[0]].red,
				                    info_ptr->palette[data[0]].green,
				                    info_ptr->palette[data[0]].blue, 0);
			}
		}
...
static rt_bool_t rtgui_image_png_load(struct rtgui_image* image, struct rtgui_filerw* file, rt_bool_t load)
{
	...
	{
		/* load all pixels */
		png->pixels = rt_malloc(image->w * image->h * sizeof(rt_uint32_t));
...
static void rtgui_image_png_blit(struct rtgui_image* image, struct rtgui_dc* dc, struct rtgui_rect* rect)
{
	struct rtgui_gdev* hwdev = rtgui_gdev_get();
	rt_uint16_t x, y, w, h;
	rt_uint32_t *ptr;
	struct rtgui_image_png* png;
	int fg_maxsample;
				...
				for(x = 0; x < w; x++)
				{
					...
					if(data[3] != 0)
					{
						rtgui_dc_draw_color_point(dc, x + rect->x1, y + rect->y1,
						                          RTGUI_ARGB(data[0], data[1], data[2], data[3]));
					}
				}
			}
		...
		case PNG_COLOR_TYPE_PALETTE:
			for(y = 0; y < h; y++)
			{
				...
				for(x = 0; x < w; x++)
				{
					...
					rtgui_dc_draw_color_point(dc, x + rect->x1, y + rect->y1,
					                          RTGUI_ARGB(png->info_ptr->palette[data[0]].red,
					                                     png->info_ptr->palette[data[0]].green,
					                                     png->info_ptr->palette[data[0]].blue, 0));
				}
			}
